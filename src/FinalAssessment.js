...
const App = (props) => {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            try {
                const data = await fetchQuestionsFromAPI(`http://localhost:5000/revisedQuestionBank/topic/${props.testTopic}`);
                console.log(`here is the data: ${data}`);
    ...
    const handleOptionClick = (selectedOption) => {
        ...
        const timeTaken = (currentTimestamp - startTimestamp) / 1000;
        const optimalTime = questionsList[currentQuestion].optimalTime;
        console.log(`Optiomal Time for this: ${optimalTime} seconds`);
        let newEfficiencyScores = null;

        if (selectedOption === questionsList[currentQuestion].answer) {
            const efficiency = Math.min(1, optimalTime / timeTaken);
